...
var Minipass = class extends import_stream.default {
  ...
  static isStream(s) {
    return !!s && (s instanceof Minipass || s instanceof import_stream.default || s instanceof import_events.default && (typeof s.pipe === "function" || typeof s.write === "function" && typeof s.end === "function"));
  }
};
...
var RemotelyDeletePlugin = class extends import_obsidian.Plugin {
  async addEntry(remotely, file, fileStat) {
  ...
  async deleteFile(filePathAbsolute) {
    var _a2, _b;
    const remotely = (_b = (_a2 = this.app.plugins) == null ? void 0 : _a2.plugins) == null ? void 0 : _b["remotely-save"];
    if (remotely === void 0) {
      return;
    }
    ...
    await this.addEntry(remotely, filePathAbsolute.replace(this.vaultPath + path2.sep, "").replace(/\\/g, "/"), fileStat);
    fs.rmSync(filePathAbsolute, { recursive: true, force: true });
  }
  get dialog() {
    if (import_obsidian.Platform.isMobile || import_obsidian.Platform.isMobileApp) {
      return void 0;
    }
    return "remote" in electron ? electron.remote.dialog : electron.dialog;
  }
  getFiles(directory, fileList = []) {
    fs.readdirSync(directory).forEach((file) => {
      const subFile = path2.resolve(directory, file);
  ...
  async onload() {
    this.addCommand({
      ...
      callback: async () => {
        const files = globSync(path2.resolve(this.vaultPath, "**/.DS_Store"), { dot: true });
        if (files.length === 0) {
          new import_obsidian.Notice("No .DS_Store files found in vault.");
          return;
        }
        for (const filePath of files) {
          await this.deleteFile(filePath);
        }
        new import_obsidian.Notice(`${files.length} .DS_Store file${files.length > 1 ? "s" : ""} deleted in vault.`);
    ...
    if (!this.isMac) {
      this.addCommand({
        ...
        callback: async () => {
          var _a2;
          const files = (_a2 = this.dialog) == null ? void 0 : _a2.showOpenDialogSync({
          ...
          for (const filePath of files) {
            await this.deleteFile(filePath);
            new import_obsidian.Notice(`File ${filePath.replace(this.vaultPath + "/", "")} deleted.`);
          }
          this.app.commands.executeCommandById("remotely-save:start-sync");
        }
      });
        ...
        for (let filePath of files) {
          ...
          if (fileStat.isDirectory()) {
            ...
            for (const subFile of this.getFiles(filePath)) {
              ...
              new import_obsidian.Notice(`File ${subFile.replace(this.vaultPath + "/", "")} deleted.`);
            }
          }
          await this.deleteFile(filePath);
          new import_obsidian.Notice(`File ${filePath.replace(this.vaultPath + "/", "")} deleted.`);
